...
type Tensor[T D] interface {
	...
	// Tensor Creation
	FullLike(value T) (Tensor[T], error)
	OnesLike() (Tensor[T], error)
	ZerosLike() (Tensor[T], error)
	RandLike() (Tensor[T], error)
	RandnLike() (Tensor[T], error)

	// Shape Manipulation
	Reshape(shape Shape) (Tensor[T], error)
