...
// ToFloat converts a concrete DType value to an internal representation
// compatible with Float.
func ToFloat[T DType](v T) Float {
	return float[T]{v: v}
}

// FromFloat converts a Float value to a concrete DType.
func FromFloat[T DType](i Float) T {
	switch any(T(0)).(type) {
	case float32:
		return T(i.F32())
	case float64:
...
var _ Float = float[float32]{}
var _ Float = float[float64]{}
...
func (f float[T]) F32() float32 {
	return float32(f.v)
}

func (f float[T]) F64() float64 {
	return float64(f.v)
}
...
var _ Slice = floatSlice[float32](nil)
var _ Slice = floatSlice[float64](nil)
