...
template <>
__host__ __device__ constexpr int64_t max_value<int64_t>()
{
    return 0x7FFFFFFFFFFFFFFFLL;
}

template <>
__host__ __device__ constexpr uint32_t max_value<uint32_t>()
{
    return 0xFFFFFFFFu;
}

template <>
__host__ __device__ constexpr uint8_t max_value<uint8_t>()
{
    return 0xFFu;
}

template <>
__host__ __device__ constexpr int32_t max_value<int32_t>()
{
    return 0x7FFFFFFF;
}

template <>
__host__ __device__ constexpr int16_t max_value<int16_t>()
{
    return 0x7FFF;
}
...
#define IS_OP(TYPENAME, INDEX_TYPENAME, FN_NAME) \
    extern "C" __global__ void FN_NAME(          \
        ...
        const size_t src_dim_size,               \
        const size_t ids_dim_size,               \
        const size_t right_size) { index_select(numel, num_dims, info, ids, inp, out, left_size, src_dim_size, ids_dim_size, right_size); }
...
#define GATHER_OP(TYPENAME, INDEX_TYPENAME, FN_NAME) \
    extern "C" __global__ void FN_NAME(              \
        ...
        const size_t src_dim_size,                   \
        const size_t ids_dim_size,                   \
        const size_t right_size) { gather(numel, ids, inp, out, left_size, src_dim_size, ids_dim_size, right_size); }
...
#define IA_OP(TYPENAME, INDEX_TYPENAME, FN_NAME) \
    extern "C" __global__ void FN_NAME(          \
        ...
        const size_t src_dim_size,               \
        const size_t dst_dim_size,               \
        const size_t right_size) { index_add(ids, ids_dim_size, inp, out, left_size, src_dim_size, dst_dim_size, right_size); }
...
#define IA_OP_F8(TYPENAME, INDEX_TYPENAME, FN_NAME) \
    extern "C" __global__ void FN_NAME(             \
        ...
        const size_t src_dim_size,                  \
        const size_t dst_dim_size,                  \
        const size_t right_size) { index_add_f8(ids, ids_dim_size, inp, out, left_size, src_dim_size, dst_dim_size, right_size); }
...
#define S_OP(TYPENAME, INDEX_TYPENAME, FN_NAME) \
    extern "C" __global__ void FN_NAME(         \
        ...
        const size_t src_dim_size,              \
        const size_t dst_dim_size,              \
        const size_t right_size) { scatter(ids, inp, out, left_size, src_dim_size, dst_dim_size, right_size); }
...
#define SA_OP(TYPENAME, INDEX_TYPENAME, FN_NAME) \
    extern "C" __global__ void FN_NAME(          \
        ...
        const size_t src_dim_size,               \
        const size_t dst_dim_size,               \
        const size_t right_size) { scatter_add(ids, inp, out, left_size, src_dim_size, dst_dim_size, right_size); }
...
#define SA_OP_F8(TYPENAME, INDEX_TYPENAME, FN_NAME) \
    extern "C" __global__ void FN_NAME(             \
        ...
        const size_t src_dim_size,                  \
        const size_t dst_dim_size,                  \
        const size_t right_size) { scatter_add_f8(ids, inp, out, left_size, src_dim_size, dst_dim_size, right_size); }
