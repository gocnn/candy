...
func CastStridedF32F32(numel, numDims int, dims, stridesX, stridesY []int, x, y []float32) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
		CastF32F32(numel, x, y)
		return
	}
	for i := range numel {
		y[GetStridedIndex(i, numDims, dims, stridesY)] = x[GetStridedIndex(i, numDims, dims, stridesX)]
	}
}

// CastStridedF32F64 converts float32 to float64 with strided memory
func CastStridedF32F64(numel, numDims int, dims, stridesX, stridesY []int, x []float32, y []float64) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
		CastF32F64(numel, x, y)
		return
	}
	for i := range numel {
		y[GetStridedIndex(i, numDims, dims, stridesY)] = float64(x[GetStridedIndex(i, numDims, dims, stridesX)])
	}
}

// CastStridedF32U8 converts float32 to uint8 with strided memory
func CastStridedF32U8(numel, numDims int, dims, stridesX, stridesY []int, x []float32, y []uint8) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
		CastF32U8(numel, x, y)
		return
	}
	for i := range numel {
		y[GetStridedIndex(i, numDims, dims, stridesY)] = uint8(x[GetStridedIndex(i, numDims, dims, stridesX)])
	}
}

// CastStridedF32U32 converts float32 to uint32 with strided memory
func CastStridedF32U32(numel, numDims int, dims, stridesX, stridesY []int, x []float32, y []uint32) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
		CastF32U32(numel, x, y)
		return
	}
	for i := range numel {
		y[GetStridedIndex(i, numDims, dims, stridesY)] = uint32(x[GetStridedIndex(i, numDims, dims, stridesX)])
	}
}

// CastStridedF32I64 converts float32 to int64 with strided memory
func CastStridedF32I64(numel, numDims int, dims, stridesX, stridesY []int, x []float32, y []int64) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
...
func CastStridedF64F32(numel, numDims int, dims, stridesX, stridesY []int, x []float64, y []float32) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
		CastF64F32(numel, x, y)
		return
	}
	for i := range numel {
		y[GetStridedIndex(i, numDims, dims, stridesY)] = float32(x[GetStridedIndex(i, numDims, dims, stridesX)])
	}
}

// CastStridedF64F64 converts float64 to float64 with strided memory
func CastStridedF64F64(numel, numDims int, dims, stridesX, stridesY []int, x, y []float64) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
...
func CastStridedF64U8(numel, numDims int, dims, stridesX, stridesY []int, x []float64, y []uint8) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
		CastF64U8(numel, x, y)
		return
	}
...
func CastStridedF64U32(numel, numDims int, dims, stridesX, stridesY []int, x []float64, y []uint32) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
...
func CastStridedF64I64(numel, numDims int, dims, stridesX, stridesY []int, x []float64, y []int64) {
	if IsContiguous(numDims, dims, stridesX) && IsContiguous(numDims, dims, stridesY) {
